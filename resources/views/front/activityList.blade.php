...
@section('content')
    ...
    <script>
        var waypoint = new Waypoint({
            element: document.getElementById('overflow-scroll'),
            handler: function(direction) {
                if(direction=='down'){
                    //进行ajax请求获取下一页数据进行渲染
                   //jQuery.post(url,data,success(data, textStatus, jqXHR),dataType)
                     var number=0;
                  var content = $.post("{{url('/front/activity/show-activity?page=')}})"+number,number);
                    number++;
                    ...
                    //标题
                        ...
                        var textnode=document.createTextNode("本可视化布局程序在HTML5浏览器上运行更加完美, 能实现自动本地化保存, 即使关闭了网页, 下一次打开仍然能恢复上一次的操作");
